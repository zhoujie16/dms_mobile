...
<template>
  <view class="swiper-page-wrap" :style="{ height: height }">
    <view class="top-warp m-flex m-align-center">
      <view class="m-flex m-flex-item m-align-center m-justify-center" v-for="(tab, i) in tabs" :key="i" :class="{ active: curIndex === i }" @click="changeTab(i)">{{ tab }}</view>
    </view>
    <swiper class="swiper-wrap" :current="curIndex" @change="swiperChange">
      <!-- 昨日 -->
      <swiper-item><ReportPage :reportData="fetchParams_0"></ReportPage></swiper-item>
      <!-- 本月 -->
      <swiper-item><ReportPage :reportData="fetchParams_1"></ReportPage></swiper-item>
    </swiper>
  </view>
</template>
